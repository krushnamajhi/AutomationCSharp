...
namespace Helpers
{
    public class Util
    {
        ...
        {
            return Directory.GetParent(getRootPath()).Parent.FullName;
        }
        ...
        public Dictionary<String, String> getORElements(String ORPath, String ORFileNames)
        {
            String value = "";
            Dictionary<String, String> ObjRep = new Dictionary<String, String>();
            List<String> ORFiles = ORFileNames.Split(';').Select(x => { return x.Trim(); }).ToList();

            foreach (String ORFile in ORFiles)
            {
                try
                {
                    ...
                    Dictionary<String, String> TempObjRep = RP.GetProperties(propFile);


                    foreach (String key in TempObjRep.Keys)

                    {

                        if (ObjRep.ContainsKey(key) == false)
                        {
                            ObjRep.Add(key, TempObjRep[key]);
                        }
                        else
                        {
                            ObjRep[key] = TempObjRep[key];
                        }
                    }
